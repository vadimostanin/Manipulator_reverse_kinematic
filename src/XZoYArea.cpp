...
XZoYArea::XZoYArea() : m_X( 0 ), m_Z( 0 ), m_angle( 0 )
{
...
const float interploationSecondsMax = 10.0;
...
bool XZoYArea::on_draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
...
//	cr->translate( 0, 600 ); // replace SURFACE_HEIGHT
  ...
  if( true == startInterpolate )
  {
	  ...
	  for( const auto & originLeg : global_originalLegs )
	  {
		  ...
		  const double interpolatedAngleXZ = interpolation.apply3( originLeg->getAngleXZ(), clonedLeg->getAngleXZ(), percentage );

		  LegsMgr::get().getManipulator()->getLeg( originLeg->getLevel() )->setAngleXZ( interpolatedAngleXZ );
	  }
...
//	  drawLeg->getLeg()->print();
	...
	int vectorY = getZ() + 50 * std::sin( Utils::deg2Rad( m_angle ) );
...
//	int perpendicularLastLegInitialExpectedY = Utils::flipY( getY(), height ) + lastLegLength * std::sin( Utils::deg2Rad( m_angle - 90 ) );
	int perpendicularLastLegInitialExpectedY = getZ() + lastLegLength * std::sin( Utils::deg2Rad( m_angle - 90 ) );
...
//    cr->save();
    cr->arc( m_X, m_Z, 5, 0, 2 * M_PI);//-MPI, M_PI
...
void XZoYArea::updateDrawLegs( Legs legs )
{
	m_drawLegs.clear();
	for( const auto & leg : legs )
	{
		m_drawLegs.emplace_back( std::make_shared<DrawLegXZoY>( leg ) );
	}
}
